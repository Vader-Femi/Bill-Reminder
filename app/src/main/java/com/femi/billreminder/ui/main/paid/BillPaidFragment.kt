...
import com.femi.billreminder.databinding.FragmentBillPaidBinding
...
class BillPaidFragment : Fragment() {
    ...
    private lateinit var billViewModel: BillPaidViewModel
    ...
    private fun setupViewModel(context: Context) {
        ...
        billViewModel =
            ViewModelProvider(
                this,
                viewModelFactory
            )[BillPaidViewModel::class.java]
    }

    private fun initAdapter() {
        adapter = BillPaidAdapter({ bill ->
            val newBill = bill.copy(paid = "false")
            billViewModel.updateBill(newBill)

            view?.let {
                Snackbar.make(it, "${bill.title} has been unpaid", Snackbar.LENGTH_LONG).apply {
